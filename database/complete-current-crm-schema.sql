-- Complete Insurance CRM Database Update
-- Created for the current CRM build.
--
-- Run this file in Supabase SQL Editor to create or upgrade the database
-- for the current Insurance CRM with:
--   - team roles: super_admin, admin, team_member
--   - clients
--   - policy types
--   - policies with policy type, assignment, payment due date
--   - interaction logs
--   - indexes, triggers, and RLS hardening
--
-- IMPORTANT FOR EXISTING DATABASES:
-- If old policies or interaction logs have NULL user_id, this script will keep
-- running and show NOTICE messages. Assign those records to the correct user,
-- then set the columns NOT NULL after backfill.
...
BEGIN
    IF EXISTS (SELECT 1 FROM policies WHERE user_id IS NULL) THEN
        RAISE NOTICE 'Existing policies have NULL user_id. Backfill policies.user_id before enforcing NOT NULL.';
    ELSE
    ...
    IF EXISTS (SELECT 1 FROM interaction_logs WHERE user_id IS NULL) THEN
        RAISE NOTICE 'Existing interaction_logs have NULL user_id. Backfill interaction_logs.user_id before enforcing NOT NULL.';
    ELSE
...
VALUES
    ('General', 'Default category for policies that have not yet been classified.', TRUE),
    ('Health Insurance', 'Individual, family floater, senior citizen, and group health policies.', TRUE),
    ('Life Insurance', 'Term plans, endowment plans, ULIP, pension, and traditional life covers.', TRUE),
    ('Motor Insurance', 'Private car, two-wheeler, commercial vehicle, OD, TP, and package policies.', TRUE),
    ('Travel Insurance', 'Domestic and international travel covers for individuals and groups.', TRUE),
    ('Business Insurance', 'Fire, marine, liability, cyber, shop, office, and SME package policies.', TRUE),
    ('Personal Accident', 'Accident, disability, income protection, and accidental death covers.', TRUE)
...
-- The application uses Next.js API routes with SUPABASE_SERVICE_KEY.
-- This keeps browser clients from reading or writing database tables directly.

-- Optional role setup after your users exist:
-- UPDATE users SET role = 'super_admin' WHERE email = 'developer@example.com';
-- UPDATE users SET role = 'admin' WHERE email = 'teamlead@example.com';
-- UPDATE users SET role = 'team_member' WHERE email = 'agent@example.com';
